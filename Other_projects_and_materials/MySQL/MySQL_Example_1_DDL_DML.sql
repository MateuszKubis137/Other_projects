...
SELECT a.name, release_year, sum(length) AS duration 
...
WHERE id = 4;
# Insert a record for your favorite Band and two of their Albums
...
SELECT a.name, s.name, max(length) longest_song 
FROM songs s 
JOIN albums a ON s.album_id = a.id 
GROUP BY album_id 
ORDER BY longest_song DESC;
# Get the number of Songs for each Band
SELECT b.name, count(s.id) number_of_songs 
